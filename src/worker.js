...
Щоб додати ще один телефон, натисніть «🔗 Підключити PWA» і отримайте новий код.`,
  ...
  async ownerMeta() {
    ...
    return json({ ok: true, updateName, maintenance, maintenanceMessage, cacheRevision, seasonalStickerTesting, updatedAt: new Date().toISOString() });
  }
  ...
  async ownerMetaUpdate(body) {
    ...
    const cacheRevision = Number(await this.state.storage.get('owner-cache-revision') || 1);
    const seasonalStickerTesting = Boolean(await this.state.storage.get('owner-seasonal-sticker-testing') || false);
    return json({ ok: true, updateName: currentName, maintenance, maintenanceMessage, cacheRevision, seasonalStickerTesting, updatedAt: new Date().toISOString() });
  }
