const APP_VERSION = '10.1.7-brand-asset-fix';
const DEPLOY_MARKER = 'myhabbit-brand-asset-fix-2026-07-31';
...
export default {
  async fetch(request, env, ctx) {
    ...
    // Backward-compatible alias for an older direct brand-image URL.
    // The canonical file remains /assets/myhabbit-brand.webp.
    if (url.pathname === '/myhabbit-brand.webp') {
      const brandUrl = new URL('/assets/myhabbit-brand.webp', request.url);
      const brandResponse = await env.ASSETS.fetch(new Request(brandUrl.toString(), {
        method: 'GET',
        headers: request.headers
      }));
      const headers = new Headers(brandResponse.headers);
      ...
      return new Response(brandResponse.body, {
        status: brandResponse.status,
        statusText: brandResponse.statusText,
        headers
      });
    }
