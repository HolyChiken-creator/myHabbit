...
function initOwnerFolds(){
  ...
  configs.forEach(([id,title,subtitle],index)=>{
    ...
    const fold=document.createElement('div');fold.className='owner-fold'+(index===0?' open':'');
    const toggle=document.createElement('button');toggle.type='button';toggle.className='owner-fold-toggle';toggle.setAttribute('aria-expanded',index===0?'true':'false');
