const APP_VERSION = '9.5.2-online-generation-control';
...
export class TelegramStateV2 {
  ...
  async fetch(request) {
    ...
    try {
      ...
      if (url.pathname === '/recovery-store' && request.method === 'POST') return this.recoveryStore(request, await request.json().catch(() => ({})));
      if (url.pathname === '/owner-recovery' && request.method === 'POST') return this.ownerRecovery(await request.json().catch(() => ({})));
...
Щоб додати ще один телефон, натисніть «🔗 Підключити PWA» і отримайте новий код.`,
  ...
  async ownerMeta() {
    ...
    return json({ ok: true, updateName, maintenance, maintenanceMessage, updatedAt: new Date().toISOString() });
  }
  ...
  async ownerMetaUpdate(body) {
    ...
    const maintenance = Boolean(await this.state.storage.get('owner-maintenance-enabled') || false);
    const maintenanceMessage = String(await this.state.storage.get('owner-maintenance-message') || 'Ми обережно оновлюємо myHabbit. Поверніться трохи пізніше.');
    return json({ ok: true, updateName: currentName, maintenance, maintenanceMessage, updatedAt: new Date().toISOString() });
  }
  ...
  async recoveryStore(request, payload={}) {
    const lookup=text(payload.lookup,128), box=payload.box;
    let auth=await this.familyAuthorize(request);
    if(!auth&&payload?.token){const h=new Headers(request.headers);h.set('authorization','Bearer '+String(payload.token));auth=await this.familyAuthorize(new Request(request.url,{method:request.method,headers:h}));}
    if(!auth){await this.recordOwnerAction('recovery-store', false, 'Не вдалося зберегти резервну копію: користувач не авторизований', { scope: box?.scope || 'unknown', lookup: lookup.slice(0,12) });return json({error:'Сесія не авторизована. Повторно увійдіть у профіль і спробуйте ще раз.'},401);}
    if(!/^[a-f0-9]{64}$/i.test(lookup)||!box?.data){await this.recordOwnerAction('recovery-store', false, 'Відхилено некоректну резервну копію', { userId: auth.user.id, familyId: auth.family.id, scope: box?.scope || 'unknown' });return json({error:'Некоректна резервна копія'},400);}
    if(box.scope==='family'&&!['admin','owner'].includes(auth.user.role)){await this.recordOwnerAction('recovery-store', false, 'Відмовлено у створенні сімейної копії: недостатньо прав', { userId: auth.user.id, familyId: auth.family.id });return json({error:'Лише адміністратор сімʼї'},403);}
    const createdAt=new Date().toISOString();
    await this.state.storage.put(`recovery:${lookup}`,{box,createdAt,familyId:auth.family.id,userId:auth.user.id});
    await this.recordOwnerAction('recovery-store', true, box.scope==='family'?'Резервну копію сімʼї збережено':'Резервну копію профілю збережено', { scope: box.scope, userId: auth.user.id, familyId: auth.family.id, lookup: lookup.slice(0,12) });
    return json({ok:true,createdAt,scope:box.scope});
  }
  async ownerRecovery(payload={}) {
    const lookup=text(payload.lookup,128);if(!/^[a-f0-9]{64}$/i.test(lookup))return json({error:'Некоректний код'},400);
    const record=await this.state.storage.get(`recovery:${lookup}`);if(!record){await this.recordOwnerAction('recovery-read', false, 'Резервну копію за кодом не знайдено', { lookup: lookup.slice(0,12) });return json({error:'Резервну копію не знайдено'},404);}
    await this.recordOwnerAction('recovery-read', true, 'Зашифровану резервну копію отримано', { scope: record.box?.scope || 'unknown', familyId: record.familyId, userId: record.userId });
    return json({ok:true,box:record.box,createdAt:record.createdAt});
  }
...
:root{color-scheme:dark;font-family:Inter,system-ui,sans-serif}*{box-sizing:border-box}body{margin:0;background:#0b1017;color:#eef4f8}.wrap{max-width:1180px;margin:auto;padding:28px;padding-top:max(28px,env(safe-area-inset-top))}.top{display:flex;justify-content:space-between;align-items:center;gap:16px}.brand{font-size:24px;font-weight:800}.muted{color:#93a4b7}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin:24px 0}.card{background:#121a24;border:1px solid #223044;border-radius:18px;padding:20px;box-shadow:0 16px 40px #0005}.value{font-size:38px;font-weight:800;margin-top:8px}.bar{height:10px;background:#253348;border-radius:20px;overflow:hidden;margin-top:14px}.fill{height:100%;background:linear-gradient(90deg,#50e3a4,#77a9ff)}canvas{width:100%;height:320px}.actions,.control-grid{display:flex;gap:12px;flex-wrap:wrap}.control-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));margin-top:14px}button,input{border:0;border-radius:12px;padding:12px 16px;font:inherit}button{cursor:pointer;background:#5f8fff;color:white;font-weight:700;min-height:46px}.danger{background:#e75265}.danger-strong{background:#a91e34}.ghost{background:#202c3d}.success{background:#28a978}.login{max-width:420px;margin:12vh auto}.hidden{display:none!important}.status{padding:10px 14px;border-radius:12px;background:#172333}.dot{display:inline-block;width:10px;height:10px;background:#4de09a;border-radius:50%;margin-right:8px}.result{min-height:22px;margin-top:12px;color:#b9c9da}.action-log{display:grid;gap:10px;max-height:520px;overflow:auto}.action-item{display:grid;grid-template-columns:12px minmax(0,1fr) auto;gap:12px;align-items:start;padding:12px;border-radius:14px;background:#0e1620;border:1px solid #223044}.action-item .lamp{width:10px;height:10px;border-radius:50%;margin-top:5px;background:#e65c70}.action-item.ok .lamp{background:#4de09a}.action-item strong{display:block}.action-item small{color:#93a4b7}.action-details{font-family:ui-monospace,monospace;font-size:11px;color:#8fa4ba;word-break:break-word;margin-top:5px}.online-list{display:grid;gap:10px}.online-user{display:flex;justify-content:space-between;gap:16px;padding:12px;border-radius:14px;background:#0e1620;border:1px solid #223044}.online-user strong{display:block}.online-user small{color:#93a4b7}.generation-row{display:grid;grid-template-columns:1fr auto auto auto;gap:10px;align-items:center}.generation-row input{width:100%;background:#0d141d;color:#fff;border:1px solid #2b3d54}.danger-zone{border-color:#63313b;background:#1b1218}.danger-zone h2{color:#ff8e9d}.danger-zone input{width:100%;margin:10px 0;background:#0d141d;color:#fff;border:1px solid #49313a}.control-card h2{margin-top:0}.control-card p{line-height:1.5}.badge{display:inline-flex;padding:5px 9px;border-radius:999px;background:#1d3344;color:#83d9ff;font-size:12px;font-weight:800}@media(max-width:800px){.grid{grid-template-columns:1fr 1fr}.top{align-items:flex-start;flex-direction:column}}@media(max-width:500px){.grid,.control-grid{grid-template-columns:1fr}.wrap{padding:16px;padding-top:max(16px,env(safe-area-inset-top))}.actions{display:grid;width:100%}.actions button{width:100%}.value{font-size:32px}}
</style></head><body><div class="wrap"><section id="login" class="login card"><div class="brand">myHabbit Owner Console</div><p class="muted">Закрита зовнішня панель власника.</p><input id="secret" type="password" placeholder="Пароль власника" style="width:100%;margin:12px 0"><button id="loginBtn" style="width:100%">Увійти</button><p id="loginError" class="muted"></p></section><main id="dashboard" class="hidden"><div class="top"><div><div class="brand">myHabbit Owner Console</div><div class="muted"><span class="dot"></span>Система працює · <span id="updated">—</span></div></div><div class="actions"><button class="ghost" id="refresh">Оновити статистику</button><button class="ghost" id="logout">Вийти</button></div></div><div class="grid"><div class="card"><div class="muted">Користувачів онлайн</div><div class="value" id="online">0</div><div class="muted">активні за останні 2 хвилини</div></div><div class="card"><div class="muted">Створено сімей</div><div class="value"><span id="families">0</span> / <span id="maxFamilies">50</span></div><div class="bar"><div id="familyFill" class="fill" style="width:0"></div></div></div><div class="card"><div class="muted">Усього користувачів</div><div class="value" id="users">0</div></div><div class="card"><div class="muted">Покоління сервера</div><div class="value" id="generation">1</div><div class="muted">зростає після службових дій</div></div></div><section class="card"><div class="top"><div><span class="badge">Живий статус</span><h2>Фактичні користувачі онлайн</h2><p class="muted">Показуються унікальні профілі, які подавали сигнал або виконували серверну дію за останні 3 хвилини.</p></div><button class="ghost" id="refreshOnline">Оновити</button></div><div id="onlineList" class="online-list"><div class="muted">Завантаження…</div></div></section><section class="card control-card"><span class="badge">Покоління сервера</span><h2>Керування поколінням</h2><p class="muted">Це службовий номер стану сервера. Він не відкочує код Cloudflare, але дозволяє позначати міграції, перезапуски та етапи даних.</p><div class="generation-row"><input id="generationInput" type="number" min="1" max="999999" value="1"><button class="ghost" id="generationPrev">−1</button><button class="success" id="generationSet">Встановити</button><button class="ghost" id="generationNext">+1</button></div><div class="result" id="generationResult"></div></section><div class="card"><h2>Зростання сімей за 30 днів</h2><canvas id="chart" width="1100" height="320"></canvas></div><div class="control-grid"><section class="card control-card"><span class="badge">Керування даними</span><h2>Примусова синхронізація</h2><p class="muted">Обробляє всі накопичені пакети змін усіх сімей одразу. Звичайні користувачі більше не мають цієї кнопки.</p><button class="success" id="forceSync">Синхронізувати всі сімʼї</button><div class="result" id="syncResult"></div></section><section class="card control-card"><span class="badge">Сервіс</span><h2>Мʼяке оновлення сервера</h2><p class="muted">Очищає тимчасові онлайн-сесії та службові ліміти, не видаляючи сімʼї або профілі.</p><button class="danger" id="restart">Мʼяко перезапустити</button><div class="result" id="restartResult"></div></section></div><section class="card control-card"><span class="badge">Відображення версії</span><h2>Назва поточного оновлення</h2><p class="muted">Цей підпис буде майже непомітно показано внизу головного екрана користувачів.</p><input id="updateName" maxlength="80" placeholder="Наприклад: Cozy Update 9.4.8"><button class="success" id="saveUpdateName">Зберегти назву</button><div class="result" id="updateNameResult"></div></section><section class="card control-card"><span class="badge">Режим обслуговування</span><h2>Тимчасово призупинити застосунок</h2><p class="muted">Показує всім користувачам мʼякий екран технічних робіт. Дані не видаляються.</p><label><input id="maintenanceEnabled" type="checkbox"> Увімкнути режим обслуговування</label><input id="maintenanceMessage" maxlength="240" placeholder="Повідомлення для користувачів"><button class="danger" id="saveMaintenance">Застосувати режим</button><div class="result" id="maintenanceResult"></div></section><section class="card control-card"><span class="badge">Зашифроване відновлення</span><h2>Отримати JSON-сесію за резервним кодом</h2><p class="muted">Код не зберігається у відкритому вигляді. Розшифрування відбувається лише у вашому браузері.</p><input id="recoveryCode" placeholder="XXXXXX-XXXXXX-XXXXXX..."><button class="success" id="recoverSession">Отримати JSON</button><div class="result" id="recoveryResult"></div></section><section class="card control-card"><div class="top"><div><span class="badge">Журнал сервера</span><h2>Виконані сервером дії</h2><p class="muted">Тут видно успішні та невдалі резервні копії, синхронізації, відновлення й службові дії.</p></div><button class="ghost" id="refreshLog">Оновити журнал</button></div><div id="actionLog" class="action-log"><div class="muted">Завантаження…</div></div></section><section class="card danger-zone"><h2>Небезпечна зона — повний вайп</h2><p>Безповоротно видаляє з сервера всі сімʼї, профілі, запрошення, черги синхронізації та сімейні сесії. Локальні копії на телефонах можуть залишитися, але більше не матимуть чинного серверного доступу.</p><label for="wipeConfirm">Для підтвердження введіть <strong>WIPE MYHABBIT</strong></label><input id="wipeConfirm" autocomplete="off" placeholder="WIPE MYHABBIT"><button class="danger-strong" id="wipe">Скинути весь сервер до нуля</button><div class="result" id="wipeResult"></div></section></main></div><script>
...
$('recoverSession').onclick=async()=>{const code=$('recoveryCode').value.trim().toUpperCase();if(!code){$('recoveryResult').textContent='Введіть резервний код';return}const b=$('recoverSession');busy(b,true,'Отримати JSON');try{const enc=new TextEncoder();const digest=await crypto.subtle.digest('SHA-256',enc.encode('myHabbit-recovery:'+code.replace(/\s/g,'')));const lookup=Array.from(new Uint8Array(digest),x=>x.toString(16).padStart(2,'0')).join('');const d=await api('/api/owner/recovery',{method:'POST',body:JSON.stringify({lookup})});const box=d.box;const un=s=>Uint8Array.from(atob(s),c=>c.charCodeAt(0));const base=await crypto.subtle.importKey('raw',enc.encode(code),'PBKDF2',false,['deriveKey']);const key=await crypto.subtle.deriveKey({name:'PBKDF2',salt:un(box.salt),iterations:180000,hash:'SHA-256'},base,{name:'AES-GCM',length:256},false,['decrypt']);const plain=await crypto.subtle.decrypt({name:'AES-GCM',iv:un(box.iv)},key,un(box.data));const payload=JSON.parse(new TextDecoder().decode(plain));const blob=new Blob([JSON.stringify({format:'myHabbit-login-profile',version:1,exportedAt:new Date().toISOString(),account:payload.account},null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='myHabbit-recovered-session.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),2000);$('recoveryResult').textContent='JSON-сесію успішно відновлено'}catch(e){$('recoveryResult').textContent='Код неправильний або копію не знайдено'}finally{busy(b,false,'Отримати JSON');await loadActionLog()}};
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/recovery/store' && request.method === 'POST') {
      const headers=new Headers({'content-type':'application/json'});const authorization=request.headers.get('authorization');if(authorization)headers.set('authorization',authorization);return stateStub(env).fetch('https://telegram-state/recovery-store',{method:'POST',headers,body:await request.text()});
    }
    if (url.pathname === '/api/owner/recovery' && request.method === 'POST') {
      if (!await ownerAuthorized(request, env)) return json({ error: 'Unauthorized' }, 401);
      return stateStub(env).fetch('https://telegram-state/owner-recovery',{method:'POST',headers:{'content-type':'application/json'},body:await request.text()});
    }
