(() => {
  ...
  const APP_VERSION = '6.1.0-pwa-experience';
  const ONBOARDING_KEY = 'myHabbitInstallOnboardingV1';
  ...
  function detectedClientPlatform(){ return isStandalone() ? 'pwa' : (tg ? 'telegram' : 'browser'); }
  function clientStatusInfo(value=detectedClientPlatform()){ return value==='pwa' ? {icon:'📱',label:'PWA User'} : value==='telegram' ? {icon:'✈️',label:'Telegram User'} : {icon:'🌐',label:'Browser User'}; }
  ...
  function normalizeState(){
    ...
    state.users.forEach(u=>{u.hiddenFromFamily=Boolean(u.hiddenFromFamily);u.showClientStatus=u.showClientStatus!==false;u.clientPlatform=u.clientPlatform||detectedClientPlatform();u.installRewardClaimed=Boolean(u.installRewardClaimed);});
    ...
    const extra=[
      ...
      {id:'pwa_always_close',icon:'📱',title:'Завжди поруч',description:'Додати myHabbit на головний екран',rarity:'Рідкісна',target:1,progress:0},
  ...
  function shopScreen(){
    const u=currentUser();
    const adminItems=[...state.shop].sort((a,b)=>Number(Boolean(b.adminCreated))-Number(Boolean(a.adminCreated))||new Date(b.createdAt||0)-new Date(a.createdAt||0));
    const real=adminItems.map(i=>{const resourceUrl=cleanResourceUrl(i.resourceUrl);return `<article class="shop-card admin-life-item"><div class="shop-top"><span class="shop-icon">${i.icon}</span><span class="stock ${i.stock?'':'out'}">${i.stock?`Залишок ${i.stock}`:'Немає'}</span></div><span class="tag">Від адміністратора</span><h3>${i.title}</h3><p>${i.description}</p>${resourceUrl?`<a class="shop-reference-link" href="${resourceUrl.replace(/&/g,'&amp;').replace(/"/g,'&quot;')}" target="_blank" rel="noopener noreferrer" aria-label="Відкрити приклад для ${String(i.title||'позиції').replace(/[<>&"]/g,'')}">↗ Подивитися, що мається на увазі</a>`:''}<div class="shop-bottom"><span class="price">${format(i.price)} 🪙</span><button class="btn primary small" data-buy="${i.id}" ${i.stock?'':'disabled'}>Обрати</button></div></article>`}).join('');
    ...
    return shell(`<div class="section-head"><div><h2>Товари від адміністратора</h2><small class="meta">Реальні нагороди та сімейні можливості</small></div><span class="tag">Головний розділ</span></div><div class="shop-grid">${real||'<div class="card empty">Адміністратор ще не додав товари</div>'}</div><div class="section-head secondary-shop-section"><h2>Стікер-бокси</h2><span class="tag">Косметика</span></div><div class="shop-grid">${boxes}</div>`,`Магазин`,`Спочатку — корисні товари від адміністратора. Стікери та колекції розміщені нижче.`);
  }
  ...
  function modal(type){
    ...
    if(type==='install-onboarding'){
      const platform=tg?'telegram':(isIOS()?'ios':(isAndroid()?'android':'browser'));
      const content=platform==='telegram'
        ? `<div class="install-steps"><span>1</span><p>Натисніть меню <strong>⋯</strong> у Mini App.</p><span>2</span><p>Оберіть <strong>«Додати на головний екран»</strong>.</p><span>3</span><p>Підтвердьте додавання myHabbit.</p></div>`
        : platform==='ios'
        ? `<div class="install-steps"><span>1</span><p>У Safari натисніть <strong>Поділитися</strong> ⎙.</p><span>2</span><p>Оберіть <strong>«На екран Початок»</strong>.</p><span>3</span><p>Натисніть <strong>«Додати»</strong>.</p></div>`
        : `<div class="install-steps"><span>1</span><p>Натисніть кнопку нижче.</p><span>2</span><p>Підтвердьте встановлення або виберіть у меню браузера <strong>«Додати на головний екран»</strong>.</p></div>`;
      return `<div class="modal-backdrop onboarding-backdrop"><div class="modal install-onboarding"><div class="install-hero">📱</div><div class="modal-head"><h2>myHabbit завжди поруч</h2><button class="close" data-action="finish-install-onboarding">×</button></div><p>Додайте застосунок на головний екран для швидкого запуску та автономної роботи.</p>${content}<div class="install-benefits"><span>⚡ Швидкий запуск</span><span>☁️ Ваш профіль</span><span>🌿 Менше зайвих кроків</span></div><div class="modal-actions"><button class="btn" data-action="finish-install-onboarding">Пізніше</button><button class="btn primary" data-action="install-app">Додати на екран</button></div></div></div>`;
    }
    return '';
  }

  function shouldShowInstallOnboarding(){return Boolean(auth) && !isStandalone() && localStorage.getItem(ONBOARDING_KEY)!=='done';}
  function finishInstallOnboarding(){localStorage.setItem(ONBOARDING_KEY,'done');document.querySelector('.onboarding-backdrop')?.remove();}
  async function installAppFromOnboarding(){
    try{
      if(tg?.addToHomeScreen){tg.addToHomeScreen();finishInstallOnboarding();showToast('Підтвердьте додавання у Telegram');return;}
      if(deferredInstallPrompt){deferredInstallPrompt.prompt();await deferredInstallPrompt.userChoice;deferredInstallPrompt=null;finishInstallOnboarding();return;}
      showToast(isIOS()?'Виконайте кроки через меню «Поділитися»':'Відкрийте меню браузера → «Додати на головний екран»');
    }catch(e){showToast('Виконайте кроки з інструкції');}
  }
  function claimPwaInstallReward(){
    const u=currentUser();if(!u||!isStandalone()||u.installRewardClaimed)return;
    u.installRewardClaimed=true;u.clientPlatform='pwa';u.achievements=u.achievements||[];
    if(!u.achievements.includes('pwa_always_close'))u.achievements.push('pwa_always_close');
    const a=state.achievements.find(x=>x.id==='pwa_always_close');if(a)a.progress=1;
    addXp(u,100,'встановлення PWA');save();showToast('📱 «Завжди поруч» · +100 XP');
  }
  ...
  function bind(){
    ...
    document.querySelectorAll('[data-copy-family-code]').forEach(el=>el.addEventListener('click',async()=>{const code=state.family.code;try{await navigator.clipboard.writeText(code);}catch{const area=document.createElement('textarea');area.value=code;area.style.position='fixed';area.style.opacity='0';document.body.appendChild(area);area.select();document.execCommand('copy');area.remove();}showToast('Код сімʼї скопійовано');}));
  ...
  function action(name, el){
    ...
    if(name==='export-account') exportAccount(false);
    if(name==='share-account') exportAccount(true);
    if(name==='install-app') installAppFromOnboarding();
    if(name==='finish-install-onboarding') finishInstallOnboarding();
  ...
  function bindModal(){
    ...
    const drop=document.getElementById('backupDropZone');if(drop){['dragenter','dragover'].forEach(n=>drop.addEventListener(n,e=>{e.preventDefault();drop.classList.add('dragging');}));['dragleave','drop'].forEach(n=>drop.addEventListener(n,e=>{e.preventDefault();drop.classList.remove('dragging');}));drop.addEventListener('drop',e=>importAccountFile(e.dataTransfer?.files?.[0]));drop.addEventListener('click',()=>importFile?.click());}
  ...
  async function sha256b64(data){return b64(await crypto.subtle.digest('SHA-256',data));}
  async function createEncryptedBackup(){
    const password=document.getElementById('transferPassword')?.value||'';
    if(password.length<6)throw new Error('Створіть пароль від 6 символів');
    persistAccount();
    const account=loadAccounts().find(x=>x.id===accountId());
    if(!account)throw new Error('Профіль не знайдено');
    const payload={format:'myHabbit-profile',version:2,exportedAt:new Date().toISOString(),appVersion:APP_VERSION,source:detectedClientPlatform(),account};
    ...
    const fileData={format:'myHabbit-encrypted-profile',version:2,createdAt:payload.exportedAt,profileName:currentUser()?.name||'profile',source:payload.source,kdf:'PBKDF2-SHA256',iterations:180000,salt:b64(salt),iv:b64(iv),checksum:await sha256b64(encrypted),data:b64(encrypted)};
    const filename=`myHabbit-${(currentUser()?.name||'profile').replace(/[^a-zа-яіїєґ0-9_-]+/gi,'-')}-${new Date().toISOString().slice(0,10)}.json`;
    const blob=new Blob([JSON.stringify(fileData,null,2)],{type:'application/json'});
    return {blob,filename,file:new File([blob],filename,{type:'application/json'})};
  }
  function downloadBackup(blob,filename){const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=filename;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(a.href),1500);}
  async function exportAccount(share=false){
    try{
      const backup=await createEncryptedBackup();
      if(share && navigator.share && (!navigator.canShare || navigator.canShare({files:[backup.file]}))){
        await navigator.share({title:'Резервна копія myHabbit',text:'Захищений JSON для перенесення профілю myHabbit.',files:[backup.file]});
        showToast('Резервною копією поділилися');
      }else{
        downloadBackup(backup.blob,backup.filename);
        showToast(share?'Системний обмін недоступний — JSON завантажено':'Захищену копію створено');
      }
    }catch(e){if(e.name!=='AbortError')showToast(e.message||'Не вдалося створити копію');}
  }
  async function importAccountFile(file){if(!file)return;const password=document.getElementById('transferPassword')?.value||'';if(password.length<6)return showToast('Спочатку введіть пароль файлу');try{const box=JSON.parse(await file.text());if(box.format!=='myHabbit-encrypted-profile')throw new Error('Це не файл профілю myHabbit');const encrypted=unb64(box.data);if(box.checksum&&await sha256b64(encrypted)!==box.checksum)throw new Error('Файл пошкоджено: контрольна сума не збігається');const key=await deriveTransferKey(password,unb64(box.salt));const clear=await crypto.subtle.decrypt({name:'AES-GCM',iv:unb64(box.iv)},key,encrypted);const payload=JSON.parse(new TextDecoder().decode(clear));const item=payload.account;if(!item?.auth||!item?.state)throw new Error('Профіль пошкоджено');const list=loadAccounts();const i=list.findIndex(x=>x.id===item.id);if(i>=0)list[i]=item;else list.unshift(item);localStorage.setItem(ACCOUNTS,JSON.stringify(list.slice(0,10)));switchAccount(item.id);showToast('Профіль перенесено');}catch(e){showToast(e.name==='OperationError'?'Невірний пароль або пошкоджений файл':e.message);}}
  ...
  function saveProfileSettings(){const u=currentUser();if(!u)return;u.equipped.badge=document.getElementById('profileBadge')?.value||null;u.equipped.frame=document.getElementById('profileFrame')?.value||null;u.equipped.animatedFrame=document.getElementById('profileAnimatedFrame')?.value||null;u.equipped.nicknameEffect=document.getElementById('profileNicknameEffect')?.value||null;u.equipped.theme=document.getElementById('profileTheme')?.value||'light';u.showClientStatus=Boolean(document.getElementById('profileShowClientStatus')?.checked);u.clientPlatform=detectedClientPlatform();save();document.querySelector('.modal-backdrop')?.remove();render();showToast('Оформлення збережено');}
  ...
  (async()=>{
    ...
    if(shouldShowInstallOnboarding())setTimeout(()=>{if(!document.querySelector('.onboarding-backdrop')){document.body.insertAdjacentHTML('beforeend',modal('install-onboarding'));bindModal();}},700);
